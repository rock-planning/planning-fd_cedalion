...
class Heuristic : public ScalarEvaluator {
    ...
    std::vector<const Operator *> operator_bucket;
    std::vector<bool> operator_marker1; // same concept for bucket as it is used for preferred ops
    bool is_unit_cost;
protected:
    ...
    void add_to_op_bucket(const Operator *op);
...
public:
    ...
    virtual void get_operator_bucket(std::vector<const Operator *> &result);
    virtual bool operator_bucket_empty() const {return operator_bucket.empty(); }
