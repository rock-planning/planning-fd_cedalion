...
ConditionalEffectLandmarkCutHeuristic::ConditionalEffectLandmarkCutHeuristic(const Options &opts)
    : Heuristic(opts) {
    num_propositions = 2; // artificial goal and artificial precondition
}

ConditionalEffectLandmarkCutHeuristic::~ConditionalEffectLandmarkCutHeuristic() {
}

// initialization
void ConditionalEffectLandmarkCutHeuristic::initialize() {
    ...
    for (int var = 0; var < g_variable_domain.size(); var++) {
        for (int value = 0; value < g_variable_domain[var]; value++)
            propositions[var].push_back(CERelaxedProposition());
    ...
    for (int i = 0; i < g_operators.size(); i++) {
        ...
        relaxed_operator_groups.push_back(CERelaxedOperatorGroup(&op, get_adjusted_cost(op)));
    }
    // Add group for artificial goal operator.
    // This must be added before groups are used to keep pointers stable.
    relaxed_operator_groups.push_back(CERelaxedOperatorGroup(0, 0));
    for (int i = 0; i < g_operators.size(); i++)
    ...
    vector<CERelaxedProposition *> goal_op_pre, goal_op_eff;
    for (int i = 0; i < g_goal.size(); i++) {
        int var = g_goal[i].first, val = g_goal[i].second;
        CERelaxedProposition *goal_prop = &propositions[var][val];
    ...
    for (int i = 0; i < relaxed_operator_groups.size(); i++) {
        CERelaxedOperatorGroup &group = relaxed_operator_groups[i];
        for (int j = 0; j < group.size(); j++) {
            CERelaxedOperator *op = &group[j];
...
void ConditionalEffectLandmarkCutHeuristic::build_relaxed_operator(const Operator &op,
                                                  CERelaxedOperatorGroup &group) {
    const vector<Prevail> &prevail = op.get_prevail();
    const vector<PrePost> &pre_post = op.get_pre_post();
    vector<CERelaxedProposition *> precondition;
    vector<CERelaxedProposition *> effects;
    ...
    for (size_t i = 0; i < pre_post.size(); ++i) {
        ...
        if (!cond.empty()) {
            vector<CERelaxedProposition *> cond_precondition(precondition);
            vector<CERelaxedProposition *> cond_effects;
...
void ConditionalEffectLandmarkCutHeuristic::add_relaxed_operator(
    const vector<CERelaxedProposition *> &precondition,
    const vector<CERelaxedProposition *> &effects,
    CERelaxedOperatorGroup &group) {
    CERelaxedOperator relaxed_op(precondition, effects, &group);
...
void ConditionalEffectLandmarkCutHeuristic::setup_exploration_queue() {
    priority_queue.clear();

    for (int var = 0; var < propositions.size(); var++) {
        for (int value = 0; value < propositions[var].size(); value++) {
            CERelaxedProposition &prop = propositions[var][value];
...
void ConditionalEffectLandmarkCutHeuristic::setup_exploration_queue_state(const State &state) {
    for (int var = 0; var < propositions.size(); var++) {
        CERelaxedProposition *init_prop = &propositions[var][state[var]];
        enqueue_if_necessary(init_prop, 0);
    }
    enqueue_if_necessary(&artificial_precondition, 0);
}

void ConditionalEffectLandmarkCutHeuristic::first_exploration(const State &state) {
    assert(priority_queue.empty());
    setup_exploration_queue();
    setup_exploration_queue_state(state);
    while (!priority_queue.empty()) {
        pair<int, CERelaxedProposition *> top_pair = priority_queue.pop();
        int popped_cost = top_pair.first;
        CERelaxedProposition *prop = top_pair.second;
        int prop_cost = prop->h_max_cost;
        assert(prop_cost <= popped_cost);
        if (prop_cost < popped_cost)
            continue;
        const vector<CERelaxedOperator *> &triggered_operators =
            prop->precondition_of;
        for (int i = 0; i < triggered_operators.size(); i++) {
            CERelaxedOperator *relaxed_op = triggered_operators[i];
            ...
            if (relaxed_op->unsatisfied_preconditions == 0) {
                ...
                for (int j = 0; j < relaxed_op->effects.size(); j++) {
                    CERelaxedProposition *effect = relaxed_op->effects[j];
...
void ConditionalEffectLandmarkCutHeuristic::first_exploration_incremental(
    vector<CERelaxedOperator *> &cut) {
    assert(priority_queue.empty());
    ...
    */
    ...
    for (size_t i = 0; i < cut.size(); i++) {
        CERelaxedOperatorGroup *group = cut[i]->group;
        assert(group);
        for (size_t j = 0; j < group->relaxed_operators.size(); j++) {
            CERelaxedOperator relaxed_op = group->relaxed_operators[j];
            int cost = relaxed_op.h_max_supporter_cost + group->cost;
            for (int k = 0; k < relaxed_op.effects.size(); k++) {
                CERelaxedProposition *effect = relaxed_op.effects[k];
...
void ConditionalEffectLandmarkCutHeuristic::second_exploration(
    const State &state, vector<CERelaxedProposition *> &second_exploration_queue, vector<CERelaxedOperator *> &cut) {
    ...
    while (!second_exploration_queue.empty()) {
        CERelaxedProposition *prop = second_exploration_queue.back();
...
void ConditionalEffectLandmarkCutHeuristic::mark_goal_plateau(CERelaxedProposition *subgoal) {
    ...
    if (subgoal && subgoal->status != GOAL_ZONE) {
        ...
        const vector<CERelaxedOperator *> &effect_of = subgoal->effect_of;
...
void ConditionalEffectLandmarkCutHeuristic::validate_h_max() const {
#ifndef NDEBUG
    ...
    for (int i = 0; i < relaxed_operator_groups.size(); i++) {
        const CERelaxedOperatorGroup &group = relaxed_operator_groups[i];
        for (int j = 0; j < group.size(); j++) {
            const CERelaxedOperator &op = group[j];
            const vector<CERelaxedProposition *> &prec = op.precondition;
...
void ConditionalEffectLandmarkCutHeuristic::reduce_operator_costs(const vector<CERelaxedOperator *> &cut, int cut_cost) {
...
void ConditionalEffectLandmarkCutHeuristic::reset_operator_costs(const State &/*state*/) {
...
int ConditionalEffectLandmarkCutHeuristic::compute_heuristic(const State &state) {
    ...
    vector<CERelaxedOperator *> cut;
    vector<CERelaxedProposition *> second_exploration_queue;
...
static Heuristic *_parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return new ConditionalEffectLandmarkCutHeuristic(opts);
}


static Plugin<Heuristic> _plugin("cond_eff_lmcut", _parse);
