...
CELandmark::CELandmark(vector<CERelaxedOperator *> &operators, int cost) :
    vector<CERelaxedOperator *, MemoryTrackingAllocator<CERelaxedOperator *> >(operators.begin(), operators.end()),
    cost(cost) {
}

// construction and destruction
ConditionalEffectIncrementalLandmarkCutHeuristic::ConditionalEffectIncrementalLandmarkCutHeuristic(const Options &opts)
    : ConditionalEffectLandmarkCutHeuristic(opts),
...
ConditionalEffectIncrementalLandmarkCutHeuristic::~ConditionalEffectIncrementalLandmarkCutHeuristic() {
}

void ConditionalEffectIncrementalLandmarkCutHeuristic::initialize() {
    ConditionalEffectLandmarkCutHeuristic::initialize();
...
void ConditionalEffectIncrementalLandmarkCutHeuristic::reduce_operator_costs(LandmarkPtr landmark) {
    for (CELandmark::iterator it_op = landmark->begin(); it_op != landmark->end(); ++it_op) {
        CERelaxedOperatorGroup *group = (*it_op)->group;
        if (!group->marked) {
            group->cost -= landmark->cost;
            group->marked = true;
        }
    }
    for (CELandmark::iterator it_op = landmark->begin(); it_op != landmark->end(); ++it_op) {
        (*it_op)->group->marked = false;
    }
}

void ConditionalEffectIncrementalLandmarkCutHeuristic::reset_operator_costs(const State &state) {
    // Redo cost changes of current landmarks (if there are no current landmarks,
    // this will just reset all costs to base costs)
    ConditionalEffectLandmarkCutHeuristic::reset_operator_costs(state);
    if (!landmarks_belong_to_parent) {
        ...
        for (SavedLandmarks::iterator it_landmark = current_landmarks.begin(); it_landmark != current_landmarks.end(); ++it_landmark) {
            ...
            reduce_operator_costs(landmark);
        }
    }
}

bool ConditionalEffectIncrementalLandmarkCutHeuristic::compute_parent_landmarks(const State &parent_state) {
...
bool ConditionalEffectIncrementalLandmarkCutHeuristic::reach_state(const State &parent_state, const Operator &op, const State &state) {
    ...
    for (size_t i = 0; i < current_parent_landmarks.size(); ++i) {
        ...
        for (CELandmark::iterator it_op = landmark->begin(); it_op != landmark->end(); ++it_op) {
            const Operator *base_operator = (*it_op)->group->op;
            if (base_operator == &op) {
                copy_landmark = false;
                h_dirty = true;
                break;
...
void ConditionalEffectIncrementalLandmarkCutHeuristic::finished_state(const State &state, int /*f*/,
                                                     bool is_on_frontier) {
...
void ConditionalEffectIncrementalLandmarkCutHeuristic::discovered_landmark(const State &state, vector<CERelaxedOperator *> &landmark, int cost) {
    ((void) state); // Avoid 'unused parameter' warning in release build
    LandmarkPtr createdLandmark(new CELandmark(landmark, cost));
    if (landmarks_belong_to_parent) {
...
int ConditionalEffectIncrementalLandmarkCutHeuristic::compute_heuristic(const State &state) {
    assert(state.get_id() == current_state_id);
    if (ConditionalEffectLandmarkCutHeuristic::compute_heuristic(state) == DEAD_END) {
...
void ConditionalEffectIncrementalLandmarkCutHeuristic::free_up_memory(SearchSpace &/*search_space*/) {
...
static Heuristic *_parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return new ConditionalEffectIncrementalLandmarkCutHeuristic(opts);
}


static Plugin<Heuristic> _plugin("cond_eff_incremental_lmcut", _parse);
