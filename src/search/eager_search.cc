...
EagerSearch::EagerSearch(
    ...
    : SearchEngine(opts),
      ...
      partial_order_reduction_method(opts.get<POR::PORMethod*>("partial_order_reduction")),
      yahsp2(opts.get<YAHSP2 *>("yahsp2")) {
...
int EagerSearch::step() {
    ...
    if (check_goal_and_set_plan(s)) {
        if (yahsp2) { // goal state is found
            vector<const Operator *> plan;
            yahsp2->trace_path(plan, s, search_space);
            set_plan(plan);
        }
        return SOLVED;
    }
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
        ...
        if (succ_node.is_new()) {
            ...
            Heuristic *ff_heuristic = heuristics[0];
            for (int j = 0; j < heuristics.size(); ++j) {
                if (!heuristics[j]->operator_bucket_empty())
                    ff_heuristic = heuristics[j];
            }

            if (yahsp2 && !yahsp2->lookahead_is_greedy()) { // for shorter plans
                int special_h = node.get_length() + 1 + succ_h;
                ff_heuristic->set_evaluator_value(special_h);
                open_list->evaluate(0, is_preferred);
            }
            ...
            // lookahead implementation (YAHSP2)
            if (yahsp2) {
                State *goal_state = yahsp2->compute_node(
                        succ_state, search_space, *open_list, search_progress, *ff_heuristic, bound);
                if (goal_state) { // goal state is found
                    vector<const Operator *> plan;
                    yahsp2->trace_path(plan, *goal_state, search_space);
                    set_plan(plan);
                    delete (goal_state);
                    return SOLVED;
                }
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    parser.add_option<POR::PORMethod*>("partial_order_reduction",
    ...
    YAHSP2::add_options_to_parser(parser);
    ...
    if (!parser.dry_run()) {
        YAHSP2 *yahsp2 = 0;
        if (opts.get<bool>("lookahead"))
            yahsp2 = new YAHSP2(opts);

        opts.set<bool>("mpd", false);
        opts.set<YAHSP2 *>("yahsp2", yahsp2);
